    ...
    stages {
            ...
            steps {
                git branch: 'master',
                    credentialsId: 'enmadm100_private_key',
                     url: '${GERRIT_MIRROR}/OSS/ENM-Parent/SQ-Gate/com.ericsson.oss.containerisation/rlset_sg'
                sh '''
                    git remote set-url origin --push https://${GERRIT_HTTP_CREDENTIALS_FUser}@${GERRIT_CENTRAL_HTTP_E2E}/OSS/ENM-Parent/SQ-Gate/com.ericsson.oss.containerisation/rlset_sg
                '''
            }
        }
        stage('Init Common Ruleset') {
            steps {
                ...
                sh '''
                    if git status | grep 'ci/bob/eric-enm-common-ruleset.yaml' > /dev/null; then
                        git add ci/bob/eric-enm-common-ruleset.yaml
                        git commit -m "NO JIRA - Updating common ruleset file."
                    fi
                '''
            }
            post {
